...
# Operator logins are userpass, managed by opentofu/openbao/management (auth.tf).
# Two of them, because a policy binds only within its own namespace:
bao login -method=userpass -namespace=admin     username=admin  # admin policy
bao login -method=userpass -namespace=admin/pki username=admin  # pki-admin policy

# Passwords are generated by the management stack and published here:
aws secretsmanager get-secret-value \
  --secret-id openbao/cloud-native-ref/users/admin \
  --query SecretString --output text | jq
```

> The bare `bao auth -method=userpass username=admin` this used to show omitted
> the namespace, so it only ever worked against a hand-created user in the root
> namespace. Both the backend and the user are now in OpenTofu.
