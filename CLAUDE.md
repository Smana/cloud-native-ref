...
namespace is unreachable by a policy on a root identity group (ADR-0036). **There is no tenant
namespace** — the `app` one was removed with ADR-0036 for exactly that reason, so
`namespaces.tf` no longer exists on either cloud. Cluster-wide endpoints such as
`sys/storage/raft/*` are callable *only* from root, which is why anything operational belongs
there.
