...
weight: 380
description: Instructions live in AGENTS.md — root and nested per directory — and skills in .agents/skills/, the two open formats every major coding agent reads. CLAUDE.md and .claude/skills become symlinks, because Claude Code reads only its own paths. Maintaining a parallel file per tool is rejected as unmaintainable; staying Claude-only is rejected because it locks the repo to one vendor. The move also fixed 1,175 lines of rules that were silently loading into every session.
...
While auditing that configuration a second problem surfaced, which changed the urgency. All eight
rules files carried `globs:` frontmatter — the field **Cursor** uses to scope its `.mdc` rules.
Claude Code scopes on `paths:`, and its documented behaviour is that a rule *without* that field
loads unconditionally. Two of the eight had no frontmatter at all and asserted their scoping only
in prose.
...
Always-on context falls from 1,649 lines to 135, with no fact removed — 790 lines moved into nine
directory-scoped files that load only when work touches them.

## Alternatives rejected

**Keep `.claude/` and fix `globs:` → `paths:`.** This repairs the context bloat, which was the
larger of the two problems, and is a one-word change in six files. It was rejected because it
