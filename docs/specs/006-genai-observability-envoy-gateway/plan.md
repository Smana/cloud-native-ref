...
Model labels (**settled from source — CL-6**): `gen_ai_original_model` = the **client-requested** name (`xplane-qwen-coder`); `gen_ai_request_model` = the **served** name after `modelNameOverride` (`xplane-qwen-coder-sql-dpo` on the canary slice, the base model on the base slice); `gen_ai_response_model` = what the engine echoed, tracking `request_model`. Gateway label: `gateway_envoyproxy_io_owning_gateway_name` (e.g. `ai-gateway`). Emitted by the **extproc** (external-processor) workload. **Canary attribution seam (US-2/FR-003):** the extproc writes the override into the model request header and only *then* records the request-model attribute from it, so the override lands on `gen_ai_request_model` — group the requested dimension by `gen_ai_original_model` and the served/canary dimension by `gen_ai_request_model`. **This closes the SPEC-002 open question** (CL-6).
...
- **Canary label**: `gen_ai_request_model` carries the `modelNameOverride` value; `gen_ai_original_model` carries the client-requested name — the load-bearing pair for US-2/SC-004 (CL-6, settled from source).
...
- [ ] **T007** (e2e — on-cluster): send ≥50 `xplane-qwen-coder` requests with the 10% `sql-dpo` canary; assert the token-usage series in VM; determine which label carries the `modelNameOverride` value and record the canary attribution answer (closes SPEC-002 open question); confirm the dashboard renders. SC-005 if traces in scope.
      **Metric names (settled from source — CL-5, no longer a caveat):** the exported series is **`gen_ai_client_token_usage_{sum,count,bucket}`** with **NO** unit segment, and the latency histograms are `gen_ai_server_*_seconds_bucket`. `prometheus/otlptranslator` appends the OTel unit only when it is not already a token of the metric name — `token` IS a token of `gen_ai.client.token.usage`, so it is dropped; `seconds` is not a token of the latency names, so it is kept. SC-001 was right. The canary override lands on `gen_ai_request_model` (CL-6).
...
- <2026-07-12> **Metric name unit suffix — RESOLVED (CL-5)**: the earlier assumption (exporter appends `_tokens`) was **wrong** and shipped a dashboard whose token panels and `model` template variable all queried a non-existent series. `prometheus/otlptranslator` v1.0.0 drops a unit suffix that is already a token of the metric name, so the series is `gen_ai_client_token_usage_sum` — exactly as SC-001 stated. Dashboard exprs corrected; no cluster round-trip was needed to settle it.
- <2026-07-12> **Canary label — RESOLVED (CL-6)**: the earlier assumption (`gen_ai_request_model` = client-requested) was **inverted**. The override lands on `gen_ai_request_model`; the client-requested name is `gen_ai_original_model`. As shipped, the canary panels filtered on `gen_ai_request_model=<base>` and would have excluded the entire canary slice — reporting a 0% canary forever. Corrected; closes the SPEC-002 open question.
