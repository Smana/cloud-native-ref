...
> Each task has a stable ID (`T001`, `T002`, …). Before marking `[x]`, cite fresh evidence (see [`.agents/skills/ship-it/references/evidence.md`](../../../../../.agents/skills/ship-it/references/evidence.md)). Tasks marked **(oss-repo)** run in `Smana/app-wizard`; **(cnr)** run in cloud-native-ref.
