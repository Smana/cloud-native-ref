...
> The **spec** is the contract: *WHAT* we are delivering and *why*. Freeze it once approved. How we build it lives in [`plan.md`](plan.md) (which also tracks tasks and the review checklist); decisions made during filling live append-only in [`clarifications.md`](clarifications.md). This is a **phased spec** (a feature split across several PRs) — see the phase map in plan.md.
...
- **FR-004**: The wizard MUST authenticate developers and create branch, commits, and PR **with the user's own GitHub token**; the wizard holds no long-lived Git credentials and no cluster credentials. _(Refined by [CL-11](clarifications.md): authentication/authorization is Zitadel OIDC (Phase 4); the user's GitHub `repo` token is obtained via a direct, linked GitHub OAuth and used only to open the PR — Zitadel cannot supply a GitHub token. Phase 1 ships with GitHub OAuth login + an `AUTH_MODE=dev` local bypass.)_
...
- User guide the wizard complements: [docs/apps-user-guide.md](https://cnref.ogenki.io/docs/platform/developer-platform/app/)
